...
[arcpbf](https://r.esri.com/arcpbf/) is an R package that processes
[Esri FeatureCollection Protocol
...
- [`open_pbf()`](reference/open_pbf.md) will read a FeatureCollection
  `pbf` file into a raw vector
- [`read_pbf()`](reference/read_pbf.md) will read a FeatureCollection
  `pbf` file *and* process it with
- [`resp_body_pbf()`](reference/httr2.md) and
  [`resps_data_pbf()`](reference/httr2.md) process `httr2_response`
  objects with FeatureCollection pbf bodies
- [`process_pbf()`](reference/process_pbf.md) will process a raw vector
  or a list of raw vectors
- [`post_process_pbf()`](reference/post_process_pbf.md) will apply post
  processing steps to the results of
  [`process_pbf()`](reference/process_pbf.md)
  - set `use_sf = TRUE` to return an `sf` object if possible. Applied by
    default in [`read_pbf()`](reference/read_pbf.md),
    [`resp_body_pbf()`](reference/httr2.md) and
    [`resps_data_pbf()`](reference/httr2.md).
...
[`resp_body_pbf()`](reference/httr2.md). Post-processing steps are
applied by default. The arguments `post_process` and `use_sf` are `TRUE`
by default.
...
the responses are returned as a list of responses.
[`resps_data_pbf()`](reference/httr2.md) processes the responses in a
vectorized manner.
...
from. Use [`read_pbf()`](reference/read_pbf.md) to do so. Again,
...
The function [`open_pbf()`](reference/open_pbf.md) will read a pbf file
into a raw vector which can be passed to
[`process_pbf()`](reference/process_pbf.md). In general you will not
...
This raw vector can be turned into an R object using
[`process_pbf()`](reference/process_pbf.md). The output *will not* be
post processed.
...
Post-processing can be applied to the result of
[`process_pbf()`](reference/process_pbf.md) using
...
[`post_process_pbf()`](reference/post_process_pbf.md) can also be
