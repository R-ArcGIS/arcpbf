...
These results use no external dependencies. If we want to convert the named list
into a familiar sf object we will have to **post-process** it using 
`post_process_pbf()`. The `use_sf` argument, which defaults to `TRUE`, will use 
the `sf` package to return an `sf` object. 
...
The FeatureCollection pbf can return three different types of results. They can
be query results as above, or they can also be **count** or **Object ID** 
results. 
...
Whereas the Object ID result type will return a `data.frame` with a single column.
The column name is the name of the object ID field and the values are the 
object IDs as a numeric vector. 
